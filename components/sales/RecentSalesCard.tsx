import Link from "next/link";
import { recentSales } from "../../lib/selectors";
import { ROUTES } from "../../lib/routes";
import { C, FONT_HEAD, FONT_BODY } from "../../lib/theme";
import { Skeleton } from "../ui/States";
import SalesTable from "./SalesTable";
import type { LoadState } from "../../lib/types";

/** how many transactions the dashboard summary shows */
const RECENT_LIMIT = 5;

export default function RecentSalesCard({ state }: { state: LoadState }) {
  const rows = recentSales(RECENT_LIMIT);

  return (
    <section aria-labelledby="recent-sales-heading">
      <div className="flex items-center justify-between gap-3 mb-2 lg:mb-3">
        <h3
          id="recent-sales-heading"
          className="text-[13px] font-semibold sm:text-sm"
          style={{ fontFamily: FONT_HEAD, color: C.ink900 }}
        >
          Recent sales &amp; distribution
        </h3>
        <Link
          href={ROUTES.sales}
          aria-label="View all sales and distribution records"
          className="text-[11px] font-semibold shrink-0 -mr-2 -my-2 px-2 py-2 sm:text-xs sm:-my-1"
          style={{ color: C.brandInk, fontFamily: FONT_BODY }}
        >
          View all
        </Link>
      </div>

      {state === "loading" ? (
        <div className="space-y-2.5" aria-busy="true">
          {Array.from({ length: 3 }).map((_, i) => (
            <Skeleton key={i} className="h-20 rounded-xl" />
          ))}
        </div>
      ) : (
        <SalesTable
          sales={rows}
          cardBreakpoint="lg"
          emptyMessage="No recent sales or distribution records."
        />
      )}
    </section>
  );
}
